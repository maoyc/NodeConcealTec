...
class Environment:  # 传递G_list，是一个{1：[],2:[]}
    ...
    def __encode(self, edges):  # [(,),(,)]边的形式
        ...
        for i in range(len(edges) - 1):
            for j in range(i + 1, len(edges)):
                if edges[i][0] == edges[j][0] or edges[i][0] == edges[j][1] or edges[i][1] == edges[j][0] or edges[i][
                    1] == edges[j][1]:
                    continue
    ...
    def __build_deepwalk_corpus(self, graph, num_paths=10, path_length=50, alpha=0,
                                rand=random.Random(0)):  # num_paths:每个节点随机游走的次数，path_length:随机游走的长度
        walks = []
    ...
    def __accuracy(self, k_distribution_orignal, k_distribution):
        ...
        for key in sorted(k_distribution_orignal.keys()):
            acc_absolute = acc_absolute + len(
                set(k_distribution_orignal[key]) & set(k_distribution[key]))  # 精度的位置放在哪里是个关键
