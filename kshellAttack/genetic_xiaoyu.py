...
def genetic_algorithm_attack(edges, k_distribution_orignal, number_genetic=1, elitesize=1, num_total=16,
                             len_chroms=10,cross_pro=0.4, mutation_pro=0.4):
    # edges为networkx生成的edges
    edges = list(edges)
    ...
    for step in range(number_genetic):  # 例如准备遗传100次
        ...
        print("max_fit_scores:",max(fit_scores))
...
if __name__ == '__main__':
    ...
    for i in range(3, 4, 1):
        ...
        for j in range(runTimes):  # 设置算法跑几遍
            ...
            print("attack:",new_edges)
            graph_geneticattack = generate_graph(new_edges)
            nx.write_edgelist(graph_geneticattack, "ban300-attack.txt", delimiter=' ', data=False)
            ...
            k_after_attack = {value: key for key, values in dict(k_distribution_geneticattack).items() for value in
                              values}
            acc_genetic = accuracy(k_before_attack, k_after_attack)
