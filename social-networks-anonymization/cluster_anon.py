...
if __name__ == "__main__":
    ...
    # 打开txt文件
    filename = 'ba5000_cluster0.03.txt'
    G_ = nx.Graph()
    with open(filename) as file:
        for line in file:
            head, tail = [int(x) for x in line.split(' ')]
            G_.add_edge(head, tail)
    ...
    e_ = nx.eigenvector_centrality(G_)
    ...
    file = r"C:\Users\hello\Desktop\数据\ba5000_cluster_15%.xls"
    # 向表中添加数据
