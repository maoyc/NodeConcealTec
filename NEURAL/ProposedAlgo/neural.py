...
def perm_loss_decep(target_comm, IG_edgeList, deg, in_deg, e_max, comm_max, communities, subedge, subgraph, subvertices,
                    beta, target_comm_index):
    add_loss = 0
    del_loss = 0
    while (True):

        add_node, loss, max_comm = get_max_loss_node(target_comm, deg[target_comm_index], in_deg[target_comm_index],
                                                     e_max[target_comm_index], comm_max[target_comm_index])
        con_list = ext_conn(communities[max_comm], target_comm, IG_edgeList)
        add_node_2, loss_2, max_comm_2 = get_max_loss_node_2(communities[max_comm], deg[max_comm], in_deg[max_comm],
                                                             e_max[max_comm], comm_max[max_comm], con_list)
        add_loss = loss + loss_2
        best_edges = getBestDelExclBridges(target_comm, subedge, subgraph, subvertices)
        ((del_node, del_node_2), loss) = get_del_max_loss(target_comm, best_edges, deg[target_comm_index],
                                                          in_deg[target_comm_index], e_max[target_comm_index], subgraph)
        del_loss = loss
...
if __name__ == "__main__":
    ...
    for i in range(0, len(communities)):
        ...
        IG_edgeList_ = perm_loss_decep(target_comm, IG_edgeList, deg, in_deg, e_max_list, comm_max_list, communities,
                                       subedge, subgraph, subvertices, beta, target_comm_index)
